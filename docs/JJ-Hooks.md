...
Installed pre-commit, commit-msg, and pre-push hooks generated by the
`pre-commit` framework are handled specially. Instead of executing those
Git-specific launchers against a temporary JJ Git view, the wrapper invokes
`pre-commit run` itself with the selected filenames and stage environment. This
avoids failures from launchers that expect a normal Git repository while
